...
public class Rule {
    ...
    private static List<String> cachePackageBlocked = new ArrayList<String>();
    ...
    public static List<String> getPackagesBlocked() {
        return new ArrayList<>(cachePackageBlocked);
    }
                ...
                try {
                    ...
                    if (all ||
                        ...
                        if(rule.wifi_blocked && rule.other_blocked) {
                            cachePackageBlocked.add(rule.packageName);
                        }
    ...
    public static boolean isPackageNetworkBlocked(String packageName, Context context) {
        SharedPreferences wifi = context.getSharedPreferences("wifi", Context.MODE_PRIVATE);
        SharedPreferences other = context.getSharedPreferences("other", Context.MODE_PRIVATE);

        return wifi.getBoolean(packageName, false) && other.getBoolean(packageName, false);
    }

    public static void blockPackageNetwork(String packageName, Context context) {
        enablePackageNetwork(packageName,false, context);
    }

    public static void unblockPackageNetwork(String packageName, Context context) {
        enablePackageNetwork(packageName,true, context);
    }

    private static void enablePackageNetwork(String packageName, boolean enabled, Context context) {
        SharedPreferences wifi = context.getSharedPreferences("wifi", Context.MODE_PRIVATE);
        SharedPreferences other = context.getSharedPreferences("other", Context.MODE_PRIVATE);

        wifi.edit().putBoolean(packageName, !enabled).apply();
        other.edit().putBoolean(packageName, !enabled).apply();

        if(enabled) {
            cachePackageBlocked.remove(packageName);
        }else {
            cachePackageBlocked.add(packageName);
        }
    }

    public static void updateRule(Context context, Rule rule, boolean root, List<Rule> listAll) {
        SharedPreferences wifi = context.getSharedPreferences("wifi", Context.MODE_PRIVATE);
        SharedPreferences other = context.getSharedPreferences("other", Context.MODE_PRIVATE);
        SharedPreferences apply = context.getSharedPreferences("apply", Context.MODE_PRIVATE);
        SharedPreferences screen_wifi = context.getSharedPreferences("screen_wifi", Context.MODE_PRIVATE);
